...
class Gateway:
    ...
    async def _flush(self):
        ...
        # Группируем по уникальному хэшу
        unique_map = {}
        for req in batch:
            ...
            if h in self.cache:
                ...
                unique_map.setdefault(h, []).append(req)

        # Параллельные вызовы для уникальных промптов
        async def call_and_distribute(h, reqs):
            try:
        ...
        tasks = [call_and_distribute(h, reqs) for h, reqs in unique_map.items()]
        if tasks:
